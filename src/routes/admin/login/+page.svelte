<script lang="ts">
  ...
  import { browser } from '$app/environment'; // <-- Tambahkan ini!
  ...
  $: errorMessage = form?.response && !form.response.success
    ? (form.response.message || 'Login gagal, periksa username dan password')
    : null;

  // Redirect hanya jika di browser dan login sukses
  $: if (browser && form?.response?.success) {
