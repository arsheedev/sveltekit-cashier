<script lang="ts">
  ...
  const typeStuffs = data?.typeStuffs || [];
  ...
  function getCategoryName(id: string | number) {
    const category = typeStuffs.find(t => t.id.toString() === id.toString());
    return category?.name || 'Unknown';
  }

  function getProductType(product: any) {
    if (product.typeId) return product.typeId.toString();
    if (product.type) {
      const category = typeStuffs.find(t => 
        t.name.toLowerCase() === product.type.toLowerCase()
      );
  ...
  let filteredItems = $derived(
    products.filter((item) => {
      ...
      const matchesCategory = $activeCategory === 'all' || productType === $activeCategory;
        ...
        <div class="card-content">
          ...
          <span class="type-badge">
            {#each typeStuffs as typeStuff}
              {#if typeStuff.id.toString() === getProductType(item)}
                {typeStuff.name}
              {/if}
            {/each}
          </span>
          <div class="card-footer">
            <span class="price">SGD {formatSGD(item.price)}</span>
            <button
              class="add-button"
...
{#if showCartPopup}
  ...
  <div class="cart-popup" transition:slide={{ duration: 300 }}>
      ...
      {#each cart as item (item.id)}
        <div class="cart-item">
          ...
          <div class="cart-item-info">
            <h4>{item.name}</h4>
            <span class="cart-item-price">SGD {formatSGD(item.price)} each</span>
          </div>
    ...
    <div class="cart-total-popup">
      <span>Total</span>
      <span class="total-price">SGD {formatSGD(cartTotalCents)}</span>
    </div>
